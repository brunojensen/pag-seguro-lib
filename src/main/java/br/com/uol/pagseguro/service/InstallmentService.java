...
public class InstallmentService {
    ...
    private static String buildInstallmentsRequestUrl(final ConnectionData connectionData) //
        throws PagSeguroServiceException {
        return connectionData.getInstallmentsUrl() + "?" + connectionData.getCredentialsUrlQuery();
    }

    public static Installments getInstallments(final Credentials credentials, //
                                               final BigDecimal amount) //
        throws PagSeguroServiceException {
        return InstallmentService.getInstallments(credentials, amount, null);
    }
    ...
    public static Installments getInstallments(final Credentials credentials, //
        ...
        throws PagSeguroServiceException {
        InstallmentService.log.info(String.format("InstallmentService.getInstallments(%1s, %2s) - begin", //
                                                  amount, //
                                                  cardBrand));

        final ConnectionData connectionData = new ConnectionData(credentials);

        final StringBuilder url = new StringBuilder(InstallmentService.buildInstallmentsRequestUrl(connectionData));
        ...
        try {
            ...
            if (httpCodeStatus == null) {
                throw new PagSeguroServiceException("Connection Timeout");
                                                                  ...
                                                                  .intValue()) {
                final Installments installments = InstallmentsParser.readInstallments(response.getInputStream());

                InstallmentService.log.info("InstallmentService.getInstallments() - end");
                ...
                InstallmentService.log.error(String.format("InstallmentService.getInstallments() - error %s", //
            ...
            InstallmentService.log.error(String.format("PaymentService.getInstallments() - error %s", //
                                                       e.getMessage()));
